(function findEscape() {
    class Graph {
        ...
        bfs(goal, root = this.vertices[0]) {
            return new Promise((resolve, reject) => { // Return a new Promise
                ...
                reject(false); // Reject the promise if no path is found
            });
        }
    ...
    g.addVertex("/UJEj.");
    g.addVertex("9tq4w0");
    g.addEdge("Fk3n//","vkCvl/"); 
    g.addEdge("vkCvl/","Fk3n//"); 
    g.addEdge("rC/1U1", "vUPF80");
    g.addEdge("vUPF80", "rC/1U1");
    g.addEdge("vUPF80", "/UJEj.");
    g.addEdge("/UJEj.", "vUPF80");
    g.addEdge("/UJEj.", "9tq4w0");
    g.addEdge("9tq4w0", "/UJEj.");
    ...
    function deliver() {
        ...
        rooms.forEach((x, i) => {
            ...
            if (prev == null) {} else {
                const queue = [];
                queue.push(`${prev}-${x}`);
                $("#input").val(window.directions[`${prev}-${x}`]);
    ...
    async function calculateRoute() {
        try {
            await g.bfs(window.startRoom, "j6ZW70");
            deliver();
        } catch (error) {
            console.error("BFS failed to find a path:", error);
        }
    }
