...
function fetchData(type) {
    $("#mudoutput").append(`<div class="line hanging-indent">Fetching ${type}...</div>`);
    $.ajax({
        type: "GET",
        beforeSend: (request) => {
            request.setRequestHeader("GMCP-Token", window.sessionToken);
        },
        url: `https://www.genesismud.org/player_file/${window.profileName}/${type}.json`,
        success: (list) => {
            ...
            if (type === "triggers") {
                ...
                $("#mudoutput").append(`<div class="line hanging-indent">Added all specified triggers to the package.</div>`);
            }
            if (type === "aliases") {
                ...
                $("#mudoutput").append(`<div class="line hanging-indent">Added all specified aliases to the package.</div>`);
            }
            if (notFoundItems.length) {
                $("#mudoutput").append(`<div style="color:red;" class="line hanging-indent">Not found ${type}: ${notFoundItems.join(", ")}</div>`);
                $("#mudoutput").append(`<div style="color:red;" class="line hanging-indent">Please try again.</div>`);
            }
            checkAndDownloadPackage();
        },
        error: (jqXHR, textStatus, errorThrown) => {
            $("#mudoutput").append(`<div style="color:red;" class="line hanging-indent">Error fetching ${type}: ${textStatus} - ${errorThrown}</div>`);
        }
    });
}
...
function checkAndDownloadPackage() {
    ...
    $("#mudoutput").append(`<div style="color:green;" class="line hanging-indent">Downloading package...</div>`);
    let packageData = {
...
function handlePackageUpload(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            $("#mudoutput").append(`<div style="color:green;" class="line hanging-indent">Error reading package file: ${error.message}</div>`);
        }
    };
    reader.readAsText(file);
    $("#mudoutput").append(`<div class="line hanging-indent">Please wait patiently and refrain from interacting with your device until all data has been successfully uploaded.</div>`);
}
...
async function uploadData(data, type) {
    ...
    if (data.length) $("#mudoutput").append(`<div style="color:green;" class="line hanging-indent">Package ${type} have all been applied.</div>`);
    $("#closesettings").click();
}
